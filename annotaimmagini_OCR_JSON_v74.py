...
def resource_path(relative_path):
    """ Restituisce il path corretto sia in sviluppo che in exe """
    try:
...
# --- CONFIGURAZIONE GLOBALE (Singola Fonte di Verità) ---
APP_VERSION = "v7.0"
APP_TITLE = "AnnotaImmagini Bounding Box e Targhe"
...
# ====================================================================
# CLASSE PER GESTIRE I LOG STATISTICHE
# ====================================================================

class UsageLogger:
    def __init__(self, folder_path, tool_version="unknown"):
        ...
        log_filename = f"{current_folder_name}_{parent_folder_name}.kpi"
        self.log_path = os.path.join(folder_path, log_filename)
        
        # QUI colleghi la variabile di istanza alla COSTANTE GLOBALE
        # Se APP_VERSION è definita nello stesso file:
        self.tool_version = APP_VERSION

        self.session_start_ts = time.time()
        self.session_start_iso = self._now_iso()

        self.current_image = None
        self.image_start_ts = None
        ...
        print("CREO FILE LOG:", self.log_path)
    ...
    def _reset_image_counters(self):
        self.image_actions = 0
        self.image_undos = 0
        self.image_redos = 0
        self.ocr_edits = 0
        self.num_letta_plate = 0
        self.num_ocr = 0
        self.ocr_validated = 0
        self.ocr_not_validated = 0
    ...
    def _write_session_start(self):
        """Scrive l'header della sessione nel file .KKK"""
        self._append("SESSION_START")
        self._append(f"folder={self.folder_path}")
        self._append(f"start_time={self.session_start_iso}")
        
        # Ora self.tool_version conterrà "v7.0" grazie all'init
        self._append(f"tool_version={self.tool_version}") 
        
        # Separatore visivo standard
        self._append("----------------------------------")



    def close_session(self):
        # chiude eventuale immagine aperta
        if self.current_image is not None:
            self.end_image()

        session_end_ts = time.time()
        total_time = session_end_ts - self.session_start_ts
        avg_time = (self.total_time_sec / self.total_images) if self.total_images > 0 else 0.0

        self._append("SESSION_END")
        self._append(f"end_time={self._now_iso()}")
        self._append(f"total_images={self.total_images}")
        self._append(f"total_time_sec={round(total_time, 2)}")
        self._append(f"avg_time_per_image_sec={round(avg_time, 2)}")

    # ------------------ IMAGE ------------------

    def start_image(self, image_name):
        # chiude la precedente se aperta
        if self.current_image is not None:
            self.end_image()

        self.current_image = image_name
        self.image_start_ts = time.time()
        self._reset_image_counters()

        self._append(f"IMAGE_START {image_name} time={self._now_iso()}")

    def end_image(self):
        if self.current_image is None:
            return

        elapsed = time.time() - self.image_start_ts

        self.total_images += 1
        self.total_time_sec += elapsed

        self._append(f"actions={self.image_actions}")
        self._append(f"undos={self.image_undos}")
        self._append(f"redos={self.image_redos}")
        self._append(f"ocr_edits={self.ocr_edits}")
        self._append(f"num_letta_plate={self.num_letta_plate}")
        self._append(f"num_ocr={self.num_ocr}")
        self._append(f"ocr_validated={self.ocr_validated}")
        self._append(f"ocr_not_validated={self.ocr_not_validated}")
        self._append(f"time_spent_sec={round(elapsed, 2)}")
        self._append(f"modified={self.image_modified}")
        self._append(f"saved={self.image_saved}")
        self._append("IMAGE_END")
        self._append("----------------------------------")
    ...
    def log_ocr_edit(self):
        self.ocr_edits += 1
        self.image_modified = True

    def log_letta_plate(self):
        self.num_letta_plate += 1
        self.image_modified = True

    def log_ocr(self, validated=False):
        self.num_ocr += 1
        if validated:
            self.ocr_validated += 1
        else:
            self.ocr_not_validated += 1
...
class BoundingBoxEditor:
    ...
    def quit_application(self, event=None):
        # chiede salvataggio se necessario
        if not self._ask_save_confirmation():
            return

        try:
            if hasattr(self, "usage_logger") and self.usage_logger:
                self.usage_logger.close_session()
        except Exception as e:
            print("Errore chiusura KPI:", e)
    ...
    def visualizza_manuale(self):
        pdf_name = "ManualeUtente.pdf"
        # Cerca il file (funziona sia in .py che in .exe grazie a resource_path)
        source_path = resource_path(pdf_name)

        if not os.path.exists(source_path):
            messagebox.showerror("Errore", f"Il file '{pdf_name}' non è stato trovato all'interno del programma.\nPercorso cercato: {source_path}")
            return

        try:
            # Crea una cartella temporanea ed estrai lì il PDF per aprirlo
            temp_dir = tempfile.mkdtemp()
            dest_path = os.path.join(temp_dir, pdf_name)
            shutil.copy2(source_path, dest_path)
            
            # Apre il file col visualizzatore predefinito di Windows
            os.startfile(dest_path)
        except Exception as e:
            messagebox.showerror("Errore", f"Impossibile aprire il manuale: {e}")
    ...
    def _push_undo_state(self, action="Modifica"):
        """
        Salva uno snapshot per UNDO/REDO.
        NON logga KPI e NON decide se l'immagine è stata modificata:
        queste decisioni spettano al chiamante.
        """
    ...
    def _is_editing_plate_entry(self):
        ...
        self.usage_logger.log_ocr_edit()
    ...
    def on_press_L(self, event=None):
        ...
        self.usage_logger.log_ocr(validated=False)
        ...
        self.usage_logger.log_letta_plate()
    ...
    def show_help(self, event=None):
        ...
        help_text = (
            ...
            "• Ctrl+q/Q: QUIT (Chiusura Applicativo)\n"
    ...
    def show_about(self, event=None):
        """Mostra finestra About con foto autore, versione globale e changelog."""
        ...
        top.title(f"Info - {APP_TITLE}") # Titolo finestra coerente
        
        # Setup Geometria e Stile
        top.geometry("1000x800")
        top.configure(bg=self.BG_LIGHT)
        top.resizable(True, True)
        
        # Tenta di aprirla "Massimizzata"
        try:
            top.state('zoomed')
        except:
            pass 
        ...
        try:
            ...
            lbl_img.image = tk_photo # Mantieni reference per evitare garbage collection
        ...
        # --- Info Testuali (Titolo e Versione) ---
        info_text_frame = tk.Frame(header_frame, bg=self.BG_LIGHT)
        info_text_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # 1. Usa il Titolo Globale
        tk.Label(info_text_frame, text=APP_TITLE, 
                 font=("Arial", 22, "bold"),
                 bg=self.BG_LIGHT, fg="white", anchor="w").pack(fill=tk.X)
        
        # 2. Usa la Versione Globale (MODIFICA CHIAVE)
        # Non definisco più VERSION qui dentro, uso APP_VERSION
        tk.Label(info_text_frame, text=f"Versione: {APP_VERSION}", 
                 font=("Arial", 14, "bold"),
        ...
        # Caricamento contenuto Changelog
        ...
        txt_changelog.config(state="disabled") # Blocca modifiche
    ...
    def __init__(self, root, folder, save_mode, filter_class=None):
        ...
        # --- AGGIUNGI QUESTA RIGA ---
        self.usage_logger = UsageLogger(self.folder)
        # ----------------------------
        ...
        self.root.title(f"{APP_TITLE} Vers {APP_VERSION} - Dark Mode")
        try:
    ...
    def create_widgets(self):
        ...
        # Contenitore per la riga unica orizzontale
        plate_line_frame = tk.Frame(ocr_controls, bg=self.BG_LIGHT)
        plate_line_frame.pack(fill=tk.X, pady=2) 
        
        # 1. Casella di testo (Targa)
        self.plate_entry = tk.Entry(plate_line_frame, 
                                    textvariable=self.plate_var, 
                                    bg="white", 
                                    fg="black", 
                                    font=("Arial", 11, "bold"), # Font leggermente ridotto per spazio
                                    state='disabled',
                                    width=13) # Larghezza ridotta per far stare tutto
        self.plate_entry.pack(side=tk.LEFT, padx=(10, 5)) 
        
        # 2. Checkbox "Valida" (SENZA expand=True)
        self.validate_plate_check = tk.Checkbutton(plate_line_frame, 
                               ...
                               font=("Arial", 9), 
                               state='disabled')
        # IMPORTANTE: Qui abbiamo tolto expand=True e fill=tk.X
        self.validate_plate_check.pack(side=tk.LEFT, padx=2)

        # 3. Pulsante Manuale
        self.btn_manuale = tk.Button(
            plate_line_frame,  
            text="📖 Manuale", 
            bg="#d9d9d9",
            fg="black",
            font=("Arial", 8, "bold"),
            cursor="hand2",
            width=10, # Larghezza fissa per stabilità
            command=self.visualizza_manuale
        )
        self.btn_manuale.pack(side=tk.LEFT, padx=(2, 0))
        
        # 4. Pulsante CHIUSURA applicativo
        self.btn_exit = tk.Button(
            plate_line_frame,
            text="❌",
            bg="#d9534f",          # rosso soft (danger)
            fg="white",
            font=("Arial", 9, "bold"),
            cursor="hand2",
            width=4,              # compatto, vicino al Manuale
            command=self.quit_application
        )
        self.btn_exit.pack(side=tk.LEFT, padx=(4, 0))

        CreateToolTip(self.btn_exit, "Chiudi applicazione (Ctrl+Q)")
    ...
    def set_current_class(self, cls):
        ...
        # Caso: nessun box selezionato → solo default
        if self.current_box == -1 or self.current_box >= len(self.bboxes):
            self.status_label.config(
                text=f"Classe predefinita impostata su {cls}. Premi 'n' per un nuovo box.",
                fg='blue'
            )
            self._update_current_box_info()
            return
        ...
        # 🔒 NESSUN CAMBIAMENTO → nessun undo, nessun KPI
        if old_cls == new_cls:
            self.status_label.config(
                text=f"Classe box {self.current_box+1} già impostata su {cls}",
                fg='gray'
            )
            self._update_current_box_info()
            return

        # ✅ DA QUI IN POI È UN’AZIONE REALE
        self._push_undo_state(f"Cambio classe → {new_cls}")
        ...
        if str(old_cls).lower().startswith('letta_plate'):
            ...
            # CASO A: Targa → Targa (rinomina OCR)
            ...
            # CASO B: Targa → NON-Targa (rimozione OCR)
            else:
                self.bboxes = [
                    b for b in self.bboxes
                    if b.get('class') != old_ocr_target
                ]

                if hasattr(self, 'loaded_ocr_boxes'):
                    self.loaded_ocr_boxes = [
                        o for o in self.loaded_ocr_boxes
                        if o.get('class', '').lower() != old_ocr_target.lower()
                    ]
        # --- FINE LOGICA INTELLIGENTE OCR ---

        # assegna la nuova classe al box selezionato
        self.bboxes[self.current_box]['class'] = new_cls
        ...
        self.status_label.config(
            text=f"Classe box {self.current_box+1} impostata su {new_cls}",
            fg='blue'
        )
    ...
    def load_image(self, event=None):
        ...
        # --- CORREZIONE: Usa 'self.folder' invece di 'self.folder_path' ---
        path = os.path.join(self.folder, self.images[self.index])
        # ------------------------------------------------------------------
        
        image_name = os.path.basename(path)
        self.usage_logger.start_image(image_name)
    ...
    def delete_current_box(self):
        ...
        if self.current_box == -1 or self.current_box >= len(self.bboxes):
            self.status_label.config(
                text="Nessun box selezionato da eliminare.",
                fg='orange'
            )
            return

        # 🔒 Salva riferimenti PRIMA di qualsiasi modifica
        box_to_delete = self.bboxes[self.current_box]
        class_name = box_to_delete.get('class', '')

        # 👉 Azione reale → snapshot UNDO
        self._push_undo_state("Eliminazione box")

        # --- GESTIONE OCR ASSOCIATO ---
        ocr_target = self._ocr_class_for_plate(class_name)

        if ocr_target:
            # Rimuovi OCR dalla memoria separata
            if hasattr(self, 'loaded_ocr_boxes'):
                self.loaded_ocr_boxes = [
                    o for o in self.loaded_ocr_boxes
                    if o.get('class', '').lower() != ocr_target.lower()
                ]

            # Rimuovi eventuali box OCR visibili
            self.bboxes = [
                b for b in self.bboxes
                if b.get('class') != ocr_target
            ]
        ...
        # --- RIMOZIONE BOX PRINCIPALE ---
        # Elimina per identità, non per indice (più sicuro)
        if box_to_delete in self.bboxes:
            self.bboxes.remove(box_to_delete)

        # Aggiorna indice selezione
        if not self.bboxes:
            self.current_box = -1
        ...
        self._update_box_stats()
        self.status_label.config(
            text=f"Box di classe '{class_name}' eliminato.",
            fg='red'
        )
    ...
    def _on_mouse_down(self, event):
        ...
        # 2. Controllo resize (SOLO setup, niente undo)
        ...
        if handle:
            ...
            # 🔒 Salva coordinate iniziali per confronto a mouse_up
            self._drag_start_coords = copy.deepcopy(
                self.bboxes[self.current_box]['coords']
            )
            return

        # 3. Selezione / Move box (priorità area più piccola)
        clicked_box_index = -1
        min_area = float('inf')
    ...
    def _on_mouse_up(self, event):
        ...
        if self.creating_box:
            ...
            if self.new_box_start and self.new_box_end:
                ...
                if x_max - x_min > 5 and y_max - y_min > 5:
                    ...
                    self.status_label.config(
                        text=f"Box di classe '{self.current_class}' creato.",
                        fg='green'
                    )
                else:
                    self.current_box = -1
                    self.status_label.config(
                        text="Creazione annullata: box troppo piccolo.",
                        fg='orange'
                    )
        ...
        # 2. Finalizza MOVE / RESIZE
        if self.dragging and self.current_box != -1 and self.current_box < len(self.bboxes):
            new_coords = self.bboxes[self.current_box]['coords']

            # 🔒 Verifica se c'è stato un cambiamento reale
            if hasattr(self, "_drag_start_coords") and new_coords != self._drag_start_coords:
                action = "Resize box" if self.resizing else "Move box"
        ...
        # Ricentra la selezione per il prossimo mouse_down
        if self.current_box != -1 and self.current_box < len(self.bboxes):
            x1, y1, x2, y2 = self.bboxes[self.current_box]['coords']
            self.drag_start_x_img = (x1 + x2) / 2
            self.drag_start_y_img = (y1 + y2) / 2
    ...
    def _select_new_folder(self):
        ...
        # 1️⃣ CHIUDI LA SESSIONE PRECEDENTE E DISCONNETTI IL LOGGER
        # È fondamentale impostarlo a None per evitare scritture fantasma durante il caricamento
        if hasattr(self, "usage_logger") and self.usage_logger:
            print(f"--- Chiusura sessione Logger precedente: {self.usage_logger.log_path} ---")
            self.usage_logger.close_session()
        ...
        if new_folder:
            # 2️⃣ PULIZIA E NORMALIZZAZIONE DEL PERCORSO
            # Forziamo il percorso assoluto per evitare errori nel calcolo del nome file KPI
            self.folder = os.path.abspath(new_folder)
            ...
            # Invalida la cache dei metadata
            self.cache_valid = False
            self.metadata_cache = {}
            
            # Filtro immagini
            if hasattr(self.root, '_class_filter') and self.root._class_filter:
                self.images = [
                    img for img in self._all_images
                    if self.image_has_class_fast(img, self.root._class_filter)
                ]
            ...
            if not self.images:
                msg = f"Nessuna immagine trovata nella cartella: {self.folder}"
                if hasattr(self.root, '_class_filter') and self.root._class_filter:
                    msg = f"Nessuna immagine con classe '{self.root._class_filter}' trovata."
                messagebox.showinfo("Nessuna Immagine", msg)
                self.canvas.delete("all")
                # Non ritorniamo subito, lasciamo che il logger si inizializzi vuoto o gestiamo l'uscita
                # Se vuoi impedire il cambio cartella se vuota, metti un return qui, ma ricorda di ripristinare il vecchio logger
                return
            
            # 3️⃣ INIZIALIZZA IL NUOVO LOGGER (CRITICO)
            # Recupera la versione. Usa self.folder che ora è sicuramente un path assoluto e pulito.
            versione_tool = APP_VERSION if 'APP_VERSION' in globals() else "v7.0"
            ...
            try:
                ...
                # DEBUG CRUCIALE: Stampa dove andrà a scrivere
                print(f"✅ NUOVO LOGGER ATTIVO. File KPI target: {self.usage_logger.log_path}")
            except Exception as e:
                messagebox.showerror("Errore Logger", f"Impossibile creare il file di log:\n{e}")
                return

            # 4. Posizionamento e caricamento
            self.index = self.find_first_virgin_index()
            
            # Aggiorna UI
            self.status_label.config(text=f"Cartella: {os.path.basename(self.folder)}", fg='green')
            
            # Carica l'immagine (che userà self.usage_logger appena creato)
            self.load_image()
