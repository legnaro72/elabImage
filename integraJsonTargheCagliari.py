...
def scegli_cartella():
    ...
    root = Tk()
    root.withdraw()
    cartella = filedialog.askdirectory(title="Seleziona la cartella contenente i file JSON da aggiornare (incluse sottocartelle)")
    return cartella
...
def update_metadata_json(cartella):
    """
    Itera sui file JSON in modo ricorsivo (includendo le sottocartelle), 
    estrae i dati dal loro nome e aggiorna il loro contenuto.
    """
    # Ho cambiato il nome del log per riflettere la nuova funzionalità ricorsiva
    log_path = os.path.join(cartella, "log_update_targhe_json_ricorsivo.txt")
    
    with open(log_path, "w", encoding="utf-8") as log:
        log.write(f"LOG AGGIORNAMENTO JSON METADATI TARGA (RICORSIVO) - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        log.write(f"Cartella base analizzata: {cartella}\n\n")

        processed_count = 0
        total_json_found = 0
        
        # --- MODIFICA CHIAVE: Uso di os.walk per attraversare le sottocartelle ---
        for root, _, files in os.walk(cartella):
            
            for nome in files:
                
                # Lavora solo sui file JSON
                if not nome.lower().endswith('.json'):
                    continue
                    
                # Costruisce il percorso completo del file JSON usando 'root'
                percorso_json = os.path.join(root, nome)
                
                # Verifica che sia un file (anche se os.walk dovrebbe garantire questo)
                ...
                total_json_found += 1
                log.write(f"  Analizzo file JSON: {percorso_json}\n") # Scrivo il percorso completo nel log
                ...
                try:
                    ...
                    msg = f"✅  Metadati targa/OCR ({targa}) integrati in: {percorso_json}. {msg_read}"
                    print(msg)
                    log.write(msg + "\n")
                    processed_count += 1
